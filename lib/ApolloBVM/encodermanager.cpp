...
EncoderManager::EncoderManager(Encoder &encoder, int num_options)
    : _encoder(encoder), _selection(0), _num_options(num_options) {}

EncoderManager::EncoderManager(Encoder &encoder, int num_options, int starting)
    : _encoder(encoder), _selection(starting), _num_options(num_options) {}

void EncoderManager::start() {

  // Set the encoder to the current selection.
  _encoder.write(COUNTS_PER_STEP * _selection);
...
void EncoderManager::poll() {
  ...
  if (new_pos != *_pos_ptr) {
    ...
    if (new_pos < 0) {
      ...
      _encoder.write(_num_options * COUNTS_PER_STEP);
    ...
    *_pos_ptr = new_pos;
    ...
    for (int i = 0; i < _num_options; i++) {
      if (i * COUNTS_PER_STEP <= new_pos &&
          new_pos <= (i + 1) * COUNTS_PER_STEP) {
        _selection = i;
        break;
      }
    }
...
int EncoderManager::getSelection() { return _selection; }
...
void EncoderManager::close() { delete _pos_ptr; }
