...
public:
  BuzzerManager(int pin) : _buzzer_pin(pin) {}
...
private:
  // Variables to play buzzer.
  const int _buzzer_pin;
  bool _state = false;

  // Tone to play, use above notes.
  const int _freq = NOTE_C6;
  // Duration of alarm buzzer to play.
  const int _duration = 325;
  // Variable to hold whether we sound or not.
  bool _play = true;
};
#endif
